...
│─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ── OR ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─│
│                 │                                │                   │
│  Self-hosted path: AgentFrameworkConnector + TACServer                   │
│  ┌─────────────────────────────┐ ┌─────────────────────────────┐   │
│  │ AgentFrameworkConnector          │ │ TACServer           (TAC)   │   │
│  │                             │ │                             │   │
...
│
├── 1. AgentFrameworkConnector (PARTNER SDK)
...
**Key point:** `TACRoutes` and `AgentFrameworkConnector` never call each other. They're both plugged into TAC's channels — one from the HTTP side, one from the agent side. The channels and TAC's callback system connect them.

### What TAC provides (the building blocks)

| TAC component | What it does | Used by |
|---|---|---|
| **VoiceChannel** | Speaks Twilio's voice protocol. Handles incoming calls, manages WebSocket connections, processes ConversationRelay messages (utterances, interrupts), streams audio responses back. Fires TAC callbacks when things happen. | Created by `AgentFrameworkConnector` |
| **SMSChannel** | Speaks Twilio's SMS protocol. Processes Maestro webhooks (participant added, message created, conversation closed), sends SMS responses via Twilio API. Fires TAC callbacks when things happen. | Created by `AgentFrameworkConnector` |
...
|---|---|---|
| **AgentFrameworkConnector** | Creates channels, wires SDK-specific agent logic to TAC callbacks. No HTTP, no server. | Developers who want full control over hosting |
...
```python
# MS Agent Framework
from tac_ms_agent_framework import AgentFrameworkConnector, RuntimeServer, OmniChannelServer

# Strands
from tac_strands import AgentFrameworkConnector, RuntimeServer, OmniChannelServer

# Future partner (e.g., OpenAI)
from tac_openai import AgentFrameworkConnector, RuntimeServer, OmniChannelServer
```
...
handler = AgentFrameworkConnector(tac=tac, create_agent=my_agent_factory)
