...
│  │ AgentFrameworkConnector       │  │ RuntimeServer             │   │
...
### Interface 1: `AgentFrameworkConnector`
...
class RuntimeServer:
    ...
    def __init__(
        self,
        omnichannel: AgentFrameworkConnector,
...
Creates both `AgentFrameworkConnector` and `RuntimeServer` internally. Single constructor, single `.start()`.
...
class OmniChannelServer:
    def __init__(
        ...
        *,
        ...
        self.omnichannel = AgentFrameworkConnector(
...
`AgentFrameworkConnector` is a pure request-processing object — it has no `start()` or `stop()` methods. Lifecycle is always owned by a server: `TACServer`, `RuntimeServer`, or a custom ASGI app. To run the handler standalone, wrap it in a `TACServer`:
...
handler = AgentFrameworkConnector(                   handler = AgentFrameworkConnector(
    tac=tac,                                        tac=tac,
...
```python
# Either SDK
handler = AgentFrameworkConnector(tac=tac, create_agent=factory)
...
The `validate_webhooks` parameter flows through `AgentFrameworkConnector` and `OmniChannelServer` constructors:
...
#### `handler.py` — `AgentFrameworkConnector` (Interface 1)
...
Pure composition — creates `AgentFrameworkConnector` + `RuntimeServer`. No logic of its own.
...
Both topologies use the same `AgentFrameworkConnector` — the handler calls `agent.stream_async()` or `agent.run_async()` on the `AgentProxy` without knowing which topology is in use. The `create_agent` factory returns the appropriate proxy type based on deployment configuration.
...
AgentFrameworkConnector, RuntimeServer, OmniChannelServer
...
| `AgentFrameworkConnector._handle_message()` (SDK) | Wraps the channel-specific handler (`_handle_voice_message` / `_handle_sms`) in a try/except. On error: logs the exception, sends a user-facing error message via the channel (e.g., "Sorry, something went wrong"), and ensures cleanup runs. |
...
Some agent setup requires async work before the first request arrives — e.g., fetching knowledge base metadata (`fetch_knowledge_base_info()`), pre-warming model connections, or validating credentials. The current Azure SDK's `OmniChannelServer` accepts an `on_startup` async callback for this. The new architecture needs to handle this without adding lifecycle methods to `AgentFrameworkConnector` (which is a pure request processor).

### Where startup hooks live in each interface level

`AgentFrameworkConnector` does **not** accept a startup hook. It has no lifecycle — it processes requests, nothing more. Async initialization belongs to the layer that owns the server lifecycle:
...
handler = AgentFrameworkConnector(tac=tac, create_agent=create_agent)
...
### Why not put startup hooks on `AgentFrameworkConnector`
...
| Removed | Replaced by |
|---|---|
| `OmniChannelServer` (current) | New `OmniChannelServer` composing `AgentFrameworkConnector` + `RuntimeServer` |
| `AgentFrameworkConnector` (current) | New `AgentFrameworkConnector` that creates channels internally |
| All route creation code | TAC's `TACRoutes` |
| Channel creation boilerplate | Internal to `AgentFrameworkConnector.__init__` |
...
| Removed | Replaced by |
|---|---|
| `OmniChannelServer` (559 lines) | New `OmniChannelServer` composing `AgentFrameworkConnector` + `RuntimeServer` |
| `AgentFrameworkConnector` (current) | New `AgentFrameworkConnector` — no duplicate streaming/agent/cleanup |
...
| Issue | Resolution |
|---|---|
| `handle_message_ready` callback had wrong parameter order | Callback wired correctly in `AgentFrameworkConnector.__init__` with proper signature |
...
This ensures:
- `from tac_ms_agent_framework import AgentFrameworkConnector` works without Foundry installed.
...
`AgentFrameworkConnector` is designed for concurrent use across multiple conversations. TAC's channels are already concurrent — `VoiceChannel` handles multiple simultaneous WebSocket connections, and `SMSChannel` processes webhooks concurrently via async handlers.
...
`AgentFrameworkConnector` does not create its own spans or metrics — it delegates to whatever tracing is configured on the ASGI app. This avoids double-instrumentation and keeps the handler deployment-agnostic.
...
| **Integration tests** | Full route → handler → agent → response flow | Each SDK's `tests/integration/` | Real `TACRoutes` + `AgentFrameworkConnector` with mock agents, real ASGI test client |
...
```python
# Test that AgentFrameworkConnector wires callbacks correctly
def test_handler_wires_callbacks():
    mock_tac = Mock(spec=TAC)
    handler = AgentFrameworkConnector(tac=mock_tac, create_agent=mock_factory)
