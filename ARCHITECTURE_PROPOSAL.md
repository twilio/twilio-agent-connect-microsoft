...
| Channel creation | In `OmniChannelHandler.__init__` | In both handler and server | User creates, passes to `TACServer` |
...
**Strands:** `OmniChannelServer` and `OmniChannelHandler` are independent implementations with no shared base class. Both create channels, implement `_stream_strands_response()`, `_get_or_create_voice_agent()`, `_cleanup_voice_agent()`, and `handle_twiml_request()`. ~200 lines of identical code between the two files.

**tac-azure:** Fixed the handler/server duplication (server composes handler), but `OmniChannelHandler` still duplicates TAC's channel creation and URL construction. Its public methods (`handle_twiml_request`, `handle_websocket_connection`, `handle_sms_webhook`) are 1:1 pass-throughs to TAC channel methods.
...
- `OmniChannelHandler` public route methods — handler works through `MessageHandler`
...
- `OmniChannelHandler` — replaced by `StrandsHandler`
...
6. **PR #10 (`simplify-examples-and-add-omni-server`):** This open PR on strands-communications-twilio goes in a similar direction — deletes `OmniChannelHandler`, `OmniChannelServer`, and `AgentProxy`, replacing them with `OmniServer` backed by `BedrockAgentCoreApp`. The example simplification is good. However, it pushes agent lifecycle (caching, streaming, channel routing) entirely to userland and creates channels internally. The `OmniServer` should be reworked to accept channels and a `MessageHandler`, becoming `AgentCoreServer` as described here.
